...
export class LogService {
  ...
  log(msg: string, ...optionalParameters: any[]) {
    this.writeToLog(msg, LogLevel.All, optionalParameters);
  }

  private shouldLog(logLevel: LogLevel): boolean {
    return (
      (logLevel >= this.logLevel && logLevel !== LogLevel.Off) ||
      this.logLevel === LogLevel.All
    );
  }

  private writeToLog(msg: string, logLevel: LogLevel, parameters: any[]) {
