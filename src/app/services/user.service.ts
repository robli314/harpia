...
export class UserService {
  ...
  currentUser: Observable<User> = this._currentUserSubject
    .asObservable()
  ...
  populate(): Observable<boolean> {
    ...
    if (this._jwtService.getToken()) {
      return this._apiService.get("/api/user/me").pipe(
        map(user => {
  ...
  authenticate(credentials: any): Observable<User> {
    return this._apiService.post("/auth/local", credentials).pipe(
      flatMap(data => {
